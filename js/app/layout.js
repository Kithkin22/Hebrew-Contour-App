...
function render(){applyLanguageLayout();syncStateBundle();if(isParallelActive()){renderParallelEditors();renderDualTables();}else{renderEditor();renderTable();}renderLegendEditor();renderInclusioManager();if(isHcShellReady()||!isWorkspaceTableView())renderCommentsPanel();setTimeout(updateCommentPopover,0);if(isParallelActive())renderArcManagerParallel();else if(typeof renderArcManager==='function')renderArcManager();if(stateBundle.parallelEnabled)updateParallelAlignStatus();if(autosaveReady)autoSaveProject();scheduleEditorLayoutFix();}
...
function updateOpenRecentMenuLabel(){const btn=document.getElementById('openRecentMenuBtn');if(!btn)return;const parallel=!!(stateBundle&&stateBundle.parallelEnabled);const label=parallel?'Open Project in Active Pane':'Open Project';const chev=btn.querySelector('.file-menu-chevron');btn.textContent=label+' ';if(chev)btn.appendChild(chev);}
...
function positionHcMenuCard(card){if(!card)return;const trigger=card.querySelector('.menu-trigger')||card.querySelector('#projectMenuTrigger')||card.querySelector('.save-tools-title');if(!trigger)return;const rect=trigger.getBoundingClientRect();const left=Math.max(8,rect.left);const top=rect.bottom+4;const isFile=card.classList.contains('file-menu-card');const menuName=card.dataset.menu||'';let maxW=isFile?280:menuName==='paste'?560:menuName==='generate'?720:320;maxW=Math.min(maxW,window.innerWidth-left-12);card.style.position='fixed';card.style.left=left+'px';card.style.top=top+'px';card.style.transform='none';card.style.right='auto';card.style.width=maxW+'px';card.style.maxWidth='calc(100vw - 16px)';card.style.zIndex='5001';}
window.positionHcMenuCard=positionHcMenuCard;
function closeProjectFileMenu(){const card=document.getElementById('projectFileMenuCard');const dd=document.getElementById('projectFileMenuDropdown');const trigger=document.getElementById('projectMenuTrigger');if(card)card.classList.remove('menu-open');if(dd){dd.setAttribute('aria-hidden','true');}if(trigger)trigger.setAttribute('aria-expanded','false');document.querySelectorAll('.file-menu-has-submenu.submenu-open').forEach(el=>el.classList.remove('submenu-open'));if(!document.querySelector('.top-stack .card.compact-card.menu-open:not(.file-menu-card)')){document.body.classList.remove('top-menu-open');const backdrop=document.getElementById('topMenuBackdrop');if(backdrop){backdrop.classList.remove('show');backdrop.setAttribute('aria-hidden','true');}}}
function openProjectFileMenu(){if(typeof closeTopMenus==='function')closeTopMenus();const card=document.getElementById('projectFileMenuCard');const dd=document.getElementById('projectFileMenuDropdown');const trigger=document.getElementById('projectMenuTrigger');if(!card||!dd)return;renderProjectFileSubmenus();card.classList.add('menu-open');dd.setAttribute('aria-hidden','false');if(trigger)trigger.setAttribute('aria-expanded','true');document.body.classList.add('top-menu-open');const backdrop=document.getElementById('topMenuBackdrop');if(backdrop){backdrop.classList.add('show');backdrop.setAttribute('aria-hidden','false');}requestAnimationFrame(function(){positionHcMenuCard(card);});}
