...
function renderWordTokensInClause(words,opt){const vi=opt.vi,ci=opt.ci,pane=opt.pane,paneState=opt.paneState||state,activeKey=opt.activeKey||'',forExport=!!(opt&&opt.forExport);const selLoc=opt.selLoc||(paneState&&paneState.selected);const idsFn=opt.idsForLoc||(pane!=null?(l=>commentIdsForLocInPane(l,pane)):(l=>commentIdsForLoc(l)));const markersFn=opt.markersForLoc||(pane!=null?(l=>commentMarkersForLocInPane(l,pane)):(l=>commentMarkersForLoc(l)));let html='';words.forEach((w,wi)=>{if(isMaqafConnector(w)){const hidden=forExport?'':' aria-hidden="true"';html+=`<span class="maqaf-connector"${hidden}>${esc(w.text||MAQAF_CHAR)}</span>`;return;}ensureSelectableWordFields(w);const l=pane!=null?{v:vi,c:ci,w:wi,pane}:{v:vi,c:ci,w:wi};const ids=idsFn(l);const commentClass=ids.length?'comment-anchored':'';const activeCommentClass=(paneState.activeCommentId&&ids.includes(paneState.activeCommentId))?'comment-active':'';const wsel=selLoc&&selLoc.v===vi&&selLoc.c===ci&&selLoc.w===wi;const same=activeKey&&normalizeHebrewWord(w.text)===activeKey&&!wsel;const paneAttr=pane!=null?` data-pane="${pane}"`:'';const specials=w.specials||[];html+=`<span class="word ${wsel?'selected':''} ${same?'sameword':''} ${commentClass} ${activeCommentClass} ${w.deleted?'deleted':''} ${specials.includes('predicate')?'pred':''} ${specials.includes('subject')?'subj':''} ${w.color?'has-text-color':''} ${wordFormatClasses(w,l,forExport)}"${paneAttr} data-v="${vi}" data-c="${ci}" data-w="${wi}" style="${wordInlineStyle(w)}">${esc(w.text)}</span>${markersFn(l)}`;if(needsSpaceAfterWord(words,wi))html+=' ';});return html;}
function renderEditor(){const ed=document.getElementById('editor');const wrap=document.getElementById('editorWrap');if(wrap)wrap.classList.add('contour-document-surface');const layout=getLanguageLayout();const scriptStyle=`direction:${layout.dir};text-align:${layout.textAlign};font-family:${layout.fontFamily}`;ensureComments();function restoreArcSvgLayer(){if(typeof ensureArcSvgLayer==='function')ensureArcSvgLayer();}if(!state.verses.length){ed.classList.remove('contour-document-page','contour-page-body');if(typeof syncContourPassageTitle==='function')syncContourPassageTitle();ed.innerHTML='<div class="editor-empty-state" dir="ltr"><p class="editor-empty-title">No text loaded yet</p><p class="muted">Use <strong>Generate text</strong> or <strong>Paste text</strong> in the menu bar above.</p></div>';restoreArcSvgLayer();hideCommentPopover();return;}ed.classList.add('contour-document-page','contour-page-body');if(typeof applyDocumentPageEditorTypography==='function')applyDocumentPageEditorTypography(ed,layout);ed.innerHTML=buildContourEditorHtmlFromState();if(typeof syncContourPassageTitle==='function')syncContourPassageTitle();restoreArcSvgLayer();document.querySelectorAll('.word').forEach(el=>el.onclick=(ev)=>{ev.stopPropagation();let clicked={v:+el.dataset.v,c:+el.dataset.c,w:+el.dataset.w};if(ev.shiftKey&&locOK(state.selected)){applyBracketRange(state.selected,clicked);return;}state.selected=clicked;let ids=commentIdsForLoc(clicked);if(ids.length)state.activeCommentId=ids[0];focusContourEditor();render();});document.querySelectorAll('.comment-marker').forEach(el=>el.onclick=(ev)=>{ev.stopPropagation();activateComment(el.dataset.commentId);});}
function buildContourEditorHtmlFromState(forExport){const layout=getLanguageLayout();ensureComments();if(!state.verses.length)return '';let html='';let activeKey=selectedWordKey();const exportMode=!!forExport;const clauseCls=typeof clauseLayoutClassNames==='function'?clauseLayoutClassNames:(c,sel)=>'clause'+(sel?' selected':'');const verseCls=typeof verseBlockClassNames==='function'?verseBlockClassNames:()=>'verse-block';const refHtml=typeof contourVerseRefHtml==='function'?contourVerseRefHtml:(v,vi)=>`<div class="muted verse-ref" dir="ltr">${esc(v.ref)}</div>`;const layoutStyleFn=typeof clauseLayoutStyle==='function'?clauseLayoutStyle:null;const layoutDirFn=typeof clauseLayoutDir==='function'?clauseLayoutDir:null;state.verses.forEach((v,vi)=>{html+=`<div class="${verseCls(v)}" data-v="${vi}">`;html+=refHtml(v,vi);v.clauses.forEach((c,ci)=>{let sel=state.selected&&state.selected.v==vi&&state.selected.c==ci;const clauseDir=layoutDirFn?layoutDirFn(c,layout):layout.dir;const marginStyle=layoutStyleFn?layoutStyleFn(c,layout):'';html+=`<div class="${clauseCls(c,sel)}" data-v="${vi}" data-c="${ci}" dir="${clauseDir}" style="${marginStyle}">`;html+=renderWordTokensInClause(c.words,{vi,ci,paneState:state,activeKey,selLoc:state.selected,forExport:exportMode});html+='</div>';});html+='</div>';});return html;}
function clauseRows(){let rows=[];state.verses.forEach((v,vi)=>v.clauses.forEach((c,ci)=>{let words=joinWordsForDisplay(c.words.filter(w=>!w.deleted));let notes=c.words.filter(w=>w.note&&!isMaqafConnector(w)).map(w=>'['+w.text+'] — '+w.note).join('; ');let trans=c.words.filter(w=>w.translation&&!isMaqafConnector(w)).map(w=>'['+w.text+'] — '+w.translation).join('; ');rows.push({vi,ci,ref:v.ref,clause:ci+1,hebrew:words,indent:c.indent,notes,translation:trans,ann:c.ann||{}});}));return rows;}
...
function insertBreak(){markUndo();let l=state.selected;if(!locOK(l)||l.w===0)return;let v=l.v,c=l.c,w=l.w;let cl=state.verses[v].clauses[c];let newc={words:cl.words.slice(w),specials:[],ann:{}};if(typeof copyClauseLayoutFields==='function')copyClauseLayoutFields(cl,newc);else newc.indent=cl.indent||0;cl.words=cl.words.slice(0,w);const isLastClause=c===state.verses[v].clauses.length-1;const hasNextVerse=v+1<state.verses.length;if(isLastClause&&hasNextVerse){state.verses[v+1].clauses.unshift(newc);applyStructureRemap(mapSplitClauseCrossVerse(v,c,w));state.selected={v:v+1,c:0,w:0};}else{state.verses[v].clauses.splice(c+1,0,newc);applyStructureRemap(mapSplitClause(v,c,w));state.selected={v,c:c+1,w:0};}render();}
function isCaretAtClauseStart(loc){if(!locOK(loc))return false;const words=state.verses[loc.v].clauses[loc.c].words;return loc.w===firstSelectableWordIndex(words);}
function isCaretAtClauseEnd(loc){if(!locOK(loc))return false;const words=state.verses[loc.v].clauses[loc.c].words;return loc.w===lastSelectableWordIndex(words);}
function wouldSplitCreateEmptyClause(cl,w){return !cl.words.slice(w).some(isSelectableWord);}
function addGapBeforeClause(loc){if(!locOK(loc)||!isCaretAtClauseStart(loc))return false;const v=loc.v,c=loc.c;if(c===0&&v===0)return false;markUndo();if(c>0){stepClauseSpacingAfterUp(state.verses[v].clauses[c-1]);}else{stepVerseSpacingAfterUp(state.verses[v-1]);}state.selected=cloneLoc(loc);syncStateBundle();render();return true;}
function addGapAfterClause(loc){if(!locOK(loc))return false;markUndo();stepClauseSpacingAfterUp(state.verses[loc.v].clauses[loc.c]);state.selected=cloneLoc(loc);syncStateBundle();render();return true;}
function removeGapBeforeClause(loc){if(!locOK(loc)||!isCaretAtClauseStart(loc))return false;const v=loc.v,c=loc.c;if(c>0){const prev=state.verses[v].clauses[c-1];if(clauseSpacingAfterPx(prev)>0){markUndo();stepClauseSpacingAfterDown(prev);state.selected=cloneLoc(loc);syncStateBundle();render();return true;}}if(c===0&&v>0){const prevV=v-1;const prevClause=state.verses[prevV].clauses[state.verses[prevV].clauses.length-1];if(clauseSpacingAfterPx(prevClause)>0){markUndo();stepClauseSpacingAfterDown(prevClause);state.selected=cloneLoc(loc);syncStateBundle();render();return true;}const prevVerse=state.verses[prevV];if(normalizeVerseSpacingAfter(prevVerse.spacingAfter)!=='default'){markUndo();stepVerseSpacingAfterDown(prevVerse);state.selected=cloneLoc(loc);syncStateBundle();render();return true;}}return false;}
function handleContourEnter(){if(!locOK(state.selected))return;if(isCaretAtClauseStart(state.selected)){addGapBeforeClause(state.selected);return;}const l=state.selected;const cl=state.verses[l.v].clauses[l.c];if(isCaretAtClauseEnd(l)&&wouldSplitCreateEmptyClause(cl,l.w)){addGapAfterClause(l);return;}insertBreak();}
function mergeClauseAnnInto(target,source){if(!source||!source.ann||!Object.keys(source.ann).length)return;target.ann=target.ann||{};Object.keys(source.ann).forEach(k=>{if(target.ann[k]==null||target.ann[k]==='')target.ann[k]=source.ann[k];});}
function applyMergedClauseSpacing(prev,removed){if(!prev||!removed)return;const trailingPx=typeof removed.spacingAfterPx==='number'?removed.spacingAfterPx:null;const trailingLevel=removed.spacingAfter;if(typeof setClauseSpacingAfterPx==='function'){setClauseSpacingAfterPx(prev,0);if(trailingPx!=null&&trailingPx!==0)setClauseSpacingAfterPx(prev,trailingPx);else if(trailingLevel&&trailingLevel!=='default'){prev.spacingAfter=trailingLevel;setClauseSpacingAfterPx(prev,spacingAfterPxFromLevel(trailingLevel));}}else{if(trailingLevel&&trailingLevel!=='default')prev.spacingAfter=trailingLevel;else delete prev.spacingAfter;delete prev.spacingAfterPx;}}
function focusContourEditor(){const el=typeof isParallelActive==='function'&&isParallelActive()?document.getElementById('parallelCompareWrap'):document.getElementById('editorWrap');if(!el)return;if(!el.hasAttribute('tabindex'))el.setAttribute('tabindex','-1');el.focus({preventScroll:true});}
function mergeClauseWithPrevious(loc){if(!locOK(loc)||!isCaretAtClauseStart(loc))return false;const v=loc.v,c=loc.c;if(c===0&&v===0)return false;markUndo();const cl=state.verses[v].clauses[c];if(c>0){const prev=state.verses[v].clauses[c-1];const oldLen=prev.words.length;prev.words=prev.words.concat(cl.words);mergeClauseAnnInto(prev,cl);applyMergedClauseSpacing(prev,cl);state.verses[v].clauses.splice(c,1);applyStructureRemap(mapMergeClause(v,c,c-1,oldLen));state.selected={v,c:c-1,w:oldLen};}else{const prevV=v-1;const prevC=state.verses[prevV].clauses.length-1;const prev=state.verses[prevV].clauses[prevC];const oldLen=prev.words.length;prev.words=prev.words.concat(cl.words);mergeClauseAnnInto(prev,cl);applyMergedClauseSpacing(prev,cl);state.verses[v].clauses.splice(0,1);const verseRemoved=!state.verses[v].clauses.length;applyStructureRemap(loc=>{let r=mapCrossVerseMerge(v,prevV,prevC,oldLen)(loc);if(!r)return null;return verseRemoved?mapVerseRemoved(v)(r):r;});if(verseRemoved)state.verses.splice(v,1);state.selected={v:prevV,c:prevC,w:oldLen};}syncStateBundle();if(typeof syncAllPaneInclusioWordMarkers==='function')syncAllPaneInclusioWordMarkers();render();return true;}
function handleContourBackspace(){if(!locOK(state.selected))return;if(isCaretAtClauseStart(state.selected)){if(removeGapBeforeClause(state.selected))return;mergeClauseWithPrevious(state.selected);return;}indent(-1);}
function indent(delta){markUndo();let l=state.selected;if(!locOK(l))return;let v=l.v,c=l.c;let cl=state.verses[v].clauses[c];const step=typeof contourTabIndentStepPx==='function'?contourTabIndentStepPx():30;const curPx=typeof clauseIndentPx==='function'?clauseIndentPx(cl):((cl.indent||0)*step);if(delta<0&&curPx===0)return;if(typeof setClauseIndentPx==='function')setClauseIndentPx(cl,curPx+delta*step);else cl.indent=Math.max(0,(cl.indent||0)+delta);state.selected=cloneLoc(l);render();}
function nextLoc(l,dir){if(!state.verses.length)return null;if(!l||!state.verses[l.v]){for(let vi=0;vi<state.verses.length;vi++){for(let ci=0;ci<state.verses[vi].clauses.length;ci++){const wi=firstSelectableWordIndex(state.verses[vi].clauses[ci].words);if(isSelectableWord(state.verses[vi].clauses[ci].words[wi]))return{v:vi,c:ci,w:wi};}}return null;}let v=l.v,c=l.c,w=l.w+dir;while(v>=0&&v<state.verses.length){while(c>=0&&c<state.verses[v].clauses.length){let words=state.verses[v].clauses[c].words;while(w>=0&&w<words.length){if(isSelectableWord(words[w]))return{v,c,w};w+=dir;}if(dir>0){c++;w=0;}else{c--;if(c>=0)w=state.verses[v].clauses[c].words.length-1;}}if(dir>0){v++;c=0;w=0;}else{v--;if(v>=0){c=state.verses[v].clauses.length-1;w=state.verses[v].clauses[c].words.length-1;}}}return l;}
...
document.addEventListener('keydown',e=>{if(e.target.matches('textarea,input,[contenteditable]'))return;if(e.key==='Escape'){if(document.querySelector('.hc-aleph-flyout:not(.hidden)'))return;if(document.getElementById('modal')?.classList.contains('show'))return;if(document.getElementById('manualInspectorModal')?.classList.contains('show'))return;if(document.getElementById('exportScopeModal')?.classList.contains('show'))return;if(document.getElementById('worksheetPdfModal')?.classList.contains('show'))return;if(document.body.classList.contains('top-menu-open'))return;if(!locOK(state.selected))return;e.preventDefault();clearWordSelection();return;}if(e.key==='ArrowLeft'){e.preventDefault();moveWord(1);}else if(e.key==='ArrowRight'){e.preventDefault();moveWord(-1);}else if(e.key==='ArrowDown'){e.preventDefault();moveClause(1);}else if(e.key==='ArrowUp'){e.preventDefault();moveClause(-1);}else if(e.key==='Enter'){e.preventDefault();handleContourEnter();}else if(e.key==='Tab'){e.preventDefault();indent(1);}else if(e.key==='Backspace'){e.preventDefault();handleContourBackspace();}else if((e.ctrlKey||e.metaKey)&&e.shiftKey&&(e.key==='Delete'||e.key==='Backspace')){e.preventDefault();toggleDeletedSelected(true);}else if(e.key==='Delete'||(e.key==='Backspace'&&e.shiftKey)){e.preventDefault();toggleDeletedSelected();}else if(e.key==='p')toggleSpecial('predicate');else if(e.key==='s')toggleSpecial('subject');else if(e.key==='n'){e.preventDefault();let l=state.selected;if(locOK(l)){let w=state.verses[l.v].clauses[l.c].words[l.w];promptModal('Word Note','Add parsing or note for selected word:',w.note,v=>{markUndo();w.note=v;render();});}}else if(e.key==='t'){e.preventDefault();let l=state.selected;if(locOK(l)){let w=state.verses[l.v].clauses[l.c].words[l.w];promptModal('Word Gloss','Add gloss for selected word:',w.translation,v=>{markUndo();w.translation=v;render();});}}});
(function initContourSelectionClear(){
  if(window._contourSelectionClearInited)return;
  window._contourSelectionClearInited=true;
  document.addEventListener('click',ev=>{
    if(typeof isContourPageBackgroundClick!=='function'||!isContourPageBackgroundClick(ev.target))return;
    if(typeof clearWordSelection==='function')clearWordSelection();
  });
})();
...
document.getElementById('pasteBox').addEventListener('paste',e=>{
  ...
  const isRtl=layout.language!=='greek';
  const diag=typeof capturePasteFromClipboard==='function'
    ?capturePasteFromClipboard(html,plain,{isRtl})
    :null;
  const parsed=diag?diag.parsed:(typeof parseWordHtmlLayoutLines==='function'?parseWordHtmlLayoutLines(html,{isRtl}):null);
  let cleaned;
  ...
  if(typeof setPasteBoxProgrammaticUpdate==='function')setPasteBoxProgrammaticUpdate(true);
  box.value=box.value.slice(0,start)+cleaned+box.value.slice(end);
  box.selectionStart=box.selectionEnd=start+cleaned.length;
  if(typeof setPasteBoxProgrammaticUpdate==='function')setPasteBoxProgrammaticUpdate(false);
});
document.getElementById('pasteBox').addEventListener('input',()=>{
  if(typeof markPasteBoxEditedByUser==='function')markPasteBoxEditedByUser();
  else if(typeof clearLastWordLayoutPasteMeta==='function')clearLastWordLayoutPasteMeta();
});
...
function loadSampleText(){
  if(typeof clearPendingPasteClipboard==='function')clearPendingPasteClipboard();
...
function finishTextFromPaste(cleaned,opts){
  ...
  const layout=getLanguageLayout();
  const isRtl=layout.language!=='greek';
  if(opts.preserveLayout&&typeof ensurePendingPasteLayoutMeta==='function')ensurePendingPasteLayoutMeta(isRtl);
  const meta=opts.preserveLayout&&typeof getLastWordLayoutPasteMeta==='function'?getLastWordLayoutPasteMeta():null;
  const usedStoredHtml=!!(opts.preserveLayout&&typeof getStoredPasteClipboardHtml==='function'&&getStoredPasteClipboardHtml());
  const runParse=(importIndent)=>{
    let layoutLines=null;
    if(opts.preserveLayout){
      if(typeof getPendingPasteLayoutLines==='function'){
        layoutLines=getPendingPasteLayoutLines(importIndent,isRtl);
      }
      if(!layoutLines&&importIndent&&meta&&typeof mergeWordIndentIntoLines==='function'){
        layoutLines=mergeWordIndentIntoLines(parseLayoutPasteLines(cleaned),meta,importIndent);
      }
      if(!layoutLines)layoutLines=parseLayoutPasteLines(cleaned);
    }
    ...
    if(opts.preserveLayout&&typeof formatPasteWithLayoutSummary==='function'&&typeof updatePasteLayoutStatus==='function'){
      const n=(state&&state.verses&&state.verses[0]&&state.verses[0].clauses)?state.verses[0].clauses.length:0;
      updatePasteLayoutStatus(formatPasteWithLayoutSummary(n,meta,importIndent,usedStoredHtml),'imported');
    }
  };
...
function createTextFromPaste(opts){
  opts=opts||{};
  bindPasteTargetPane();
  if(!opts.preserveLayout)generatedRefs=[];
  const box=document.getElementById('pasteBox');
  let sourceText=box.value;
  if(opts.preserveLayout&&typeof getStoredPasteClipboardHtml==='function'&&getStoredPasteClipboardHtml()){
    const meta=typeof getLastWordLayoutPasteMeta==='function'?getLastWordLayoutPasteMeta():null;
    if(meta&&meta.text)sourceText=meta.text;
  }
  const cleaned=cleanLogosPaste(sourceText,opts.preserveLayout?{preserveBlankLines:true}:undefined);
  if(typeof setPasteBoxProgrammaticUpdate==='function')setPasteBoxProgrammaticUpdate(true);
  box.value=cleaned;
  if(typeof setPasteBoxProgrammaticUpdate==='function')setPasteBoxProgrammaticUpdate(false);
  finishTextFromPaste(cleaned,opts);
}
...
function legendEntryKey(type,color=''){const c=String(color||'').trim().toLowerCase();return type+'|'+(c||'_');}
function detectUsedLegendEntries(){ensureLegend();let add=[];const pendingKeys=new Set();function alreadyHas(type,color=''){const key=legendEntryKey(type,color);if(pendingKeys.has(key))return true;if(legendHas(type,color))return true;return add.some(e=>legendEntryKey(e.type,e.color)===key);}function queue(type,color='',label=''){if(alreadyHas(type,color))return;const key=legendEntryKey(type,color);pendingKeys.add(key);add.push({type,color,label});}let sawBold=false,sawItalic=false,sawUnderline=false,sawDouble=false,sawPred=false,sawSubj=false;const bracketColors=new Set();function scanWords(verses){verses.forEach(v=>v.clauses.forEach(c=>c.words.forEach(w=>{let f=w.format||{};if(f.highlight)queue('highlight',f.highlight,'');if(w.color)queue('textColor',w.color,'');if(w.bracketStart||w.bracketEnd)bracketColors.add(w.bracketColor||w.inclusioColor||'#ffffff');if(f.bold)sawBold=true;if(f.italic)sawItalic=true;if(f.underline)sawUnderline=true;if(f.doubleUnderline)sawDouble=true;if(w.specials&&w.specials.includes('predicate'))sawPred=true;if(w.specials&&w.specials.includes('subject'))sawSubj=true;})));}if(stateBundle.parallelEnabled){[0,1].forEach(p=>{const st=stateBundle.panes[p];if(st&&st.verses.length)scanWords(st.verses);});}else scanWords(state.verses);bracketColors.forEach(c=>queue('bracket',c,''));if(sawBold)queue('bold','#000000','');if(sawItalic)queue('italic','#000000','');if(sawUnderline)queue('underline','#000000','');if(sawDouble)queue('doubleUnderline','#000000','');if(sawPred)queue('predicate','#0b61a4','');if(sawSubj)queue('subject','#b02a2a','');state.legend=state.legend.concat(add);renderLegendEditor();syncLegendBelowEditor();autoSaveProject();if(!add.length)alert('No new used markings found.');}
function syncLegendBelowEditor(){const wrap=document.getElementById('legendBelowEditor');const contourTab=document.getElementById('contourTab');if(!wrap||!contourTab)return;const anchor=isParallelActive()?document.getElementById('parallelCompareWrap'):document.getElementById('singleEditorSection');if(anchor&&wrap.parentElement===contourTab&&wrap.previousElementSibling!==anchor)anchor.insertAdjacentElement('afterend',wrap);wrap.style.display=isWorkspaceTableView()?'none':'block';wrap.style.flexShrink='0';const head=document.getElementById('legendBelowHeader');if(head){const paneNote=stateBundle.parallelEnabled&&isParallelActive()?' ('+paneLabel(stateBundle.activePane)+')':'';const expanded=!wrap.classList.contains('collapsed');head.textContent=(expanded?'▼':'▶')+' Legend / Key'+paneNote;}}
...
function updateCurrentProjectLabel(){const el=document.getElementById('currentProjectName');const rec=getCurrentProjectRecord();if(el)el.textContent=rec?rec.name:'Untitled Project';if(typeof updateProjectHeaderName==='function')updateProjectHeaderName();}
...
function persistCurrentProject(silent){const id=projectStore.currentProjectId;if(!id)return;const now=new Date().toISOString();syncStateBundle();const payload=projectPayload();let rec=projectStore.projects[id];if(rec){rec.payload=payload;rec.updatedAt=now;if((!rec.name||rec.name==='Untitled Project')&&defaultProjectName()!=='Untitled Project')rec.name=defaultProjectName();}else{rec={id,name:defaultProjectName(),createdAt:now,updatedAt:now,appVersion:APP_VERSION,payload};projectStore.projects[id]=rec;}writeProjectStore();updateCurrentProjectLabel();renderProjectFileSubmenus();if(typeof setSaveIndicator==='function')setSaveIndicator(silent?'autosaved':'manual-saved',{at:Date.now()});if(!silent){updateSaveStatus('Saved: '+rec.name+' · '+new Date().toLocaleTimeString());}else if(typeof setSaveIndicator!=='function'){updateSaveStatus('Autosaved: '+(rec?rec.name:'project')+' · '+new Date().toLocaleTimeString());}}
function autoSaveProject(){try{persistCurrentProject(true);}catch(e){updateSaveStatus('Autosave unavailable in this browser.');}}
...
function renameCurrentProject(){if(typeof requestProjectRename==='function'){requestProjectRename();return;}const cur=getCurrentProjectRecord();if(!cur)return;promptModal('Rename project','Project name:',cur.name,name=>{const n=(name||'').trim();if(!n)return;cur.name=n;writeProjectStore();updateCurrentProjectLabel();renderProjectFileSubmenus();updateSaveStatus('Renamed to: '+n);});}
...
function isShellProjectMenu(){return document.body.classList.contains('hc-shell-ready');}
function ensureShellProjectMenuPortal(){const dd=document.getElementById('projectFileMenuDropdown');if(!dd||!isShellProjectMenu()||dd.dataset.shellPortal==='1')return;dd.dataset.shellPortal='1';document.body.appendChild(dd);dd.classList.add('file-menu-dropdown-portal');}
function positionProjectFileMenuDropdown(anchorEl){const dd=document.getElementById('projectFileMenuDropdown');if(!dd||!isShellProjectMenu())return;const anchor=anchorEl||document.querySelector('.hc-toolbar-btn')||document.getElementById('projectMenuTrigger');if(!anchor)return;const r=anchor.getBoundingClientRect();const left=Math.max(8,Math.min(r.left,window.innerWidth-280));const top=Math.min(r.bottom+4,window.innerHeight-40);dd.style.left=left+'px';dd.style.top=top+'px';}
function resetProjectFileMenuDropdownPosition(){const dd=document.getElementById('projectFileMenuDropdown');if(!dd)return;dd.style.left='';dd.style.top='';}
function closeProjectFileMenu(){const card=document.getElementById('projectFileMenuCard');const dd=document.getElementById('projectFileMenuDropdown');const trigger=document.getElementById('projectMenuTrigger');if(card)card.classList.remove('menu-open');if(dd)dd.setAttribute('aria-hidden','true');if(trigger)trigger.setAttribute('aria-expanded','false');document.body.classList.remove('hc-project-menu-open');resetProjectFileMenuDropdownPosition();document.querySelectorAll('.file-menu-has-submenu.submenu-open').forEach(el=>el.classList.remove('submenu-open'));}
function openProjectFileMenu(anchorEl){if(typeof closeTopMenus==='function')closeTopMenus();const card=document.getElementById('projectFileMenuCard');const dd=document.getElementById('projectFileMenuDropdown');const trigger=document.getElementById('projectMenuTrigger');if(!card||!dd)return;ensureShellProjectMenuPortal();renderProjectFileSubmenus();card.classList.add('menu-open');dd.setAttribute('aria-hidden','false');if(isShellProjectMenu()){positionProjectFileMenuDropdown(anchorEl);document.body.classList.add('hc-project-menu-open');}if(trigger)trigger.setAttribute('aria-expanded','true');}
function openRecentProjectsMenu(anchorEl){openProjectFileMenu(anchorEl);const btn=document.getElementById('openRecentMenuBtn');const li=btn?btn.parentElement:null;if(li)li.classList.add('submenu-open');if(btn)btn.focus();}
function toggleProjectFileMenu(anchorEl){const card=document.getElementById('projectFileMenuCard');if(card&&card.classList.contains('menu-open'))closeProjectFileMenu();else openProjectFileMenu(anchorEl);}
window.openProjectFileMenu=openProjectFileMenu;
window.openRecentProjectsMenu=openRecentProjectsMenu;
window.closeProjectFileMenu=closeProjectFileMenu;
function handleProjectFileAction(action){switch(action){case 'new-project':newProjectPrompt();break;case 'save-project':saveProjectLocal();break;case 'save-as':saveProjectAs();break;case 'export-contour-pdf':openWorksheetPdfWizard();break;case 'export-contour-word':exportContourDocx();break;case 'export-contour-side-by-side':if(typeof exportContourSideBySideDocx==='function')exportContourSideBySideDocx();break;case 'export-contour-html':exportContourHtml();break;case 'export-table-pdf':if(isParallelActive())renderDualTables();else if(typeof renderTable==='function')renderTable();if(isParallelActive())exportTablePdfParallel();else exportTablePdf();break;case 'export-table-word':if(isParallelActive())renderDualTables();else if(typeof renderTable==='function')renderTable();document.getElementById('docxExport').click();break;case 'export-project-json':downloadProjectFile();break;case 'import-text':if(typeof openTopMenu==='function')openTopMenu('paste');break;case 'import-project':document.getElementById('projectFileInput').click();break;case 'import-aleph-translation':if(typeof promptImportAlephTranslation==='function')promptImportAlephTranslation();break;case 'settings-rename':renameCurrentProject();break;case 'settings-duplicate':duplicateCurrentProject();break;case 'settings-delete':deleteCurrentProject();break;default:break;}closeProjectFileMenu();}
