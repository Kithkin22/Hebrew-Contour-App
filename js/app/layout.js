...
function buildContourEditorHtmlFromState(forExport){const layout=getLanguageLayout();const scriptStyle=`direction:${layout.dir};text-align:${layout.textAlign};font-family:${layout.fontFamily}`;ensureComments();if(!state.verses.length)return '';let html='';let activeKey=selectedWordKey();const exportMode=!!forExport;const clauseCls=typeof clauseLayoutClassNames==='function'?clauseLayoutClassNames:(c,sel)=>'clause'+(sel?' selected':'');const verseCls=typeof verseBlockClassNames==='function'?verseBlockClassNames:()=>'verse-block';const refHtml=typeof contourVerseRefHtml==='function'?contourVerseRefHtml:(v,vi)=>`<div class="muted verse-ref" dir="ltr">${esc(v.ref)}</div>`;const indentPx=typeof contourDisplayIndentPx==='function'?contourDisplayIndentPx():36;state.verses.forEach((v,vi)=>{html+=`<div class="${verseCls(v)}" data-v="${vi}">`;html+=refHtml(v,vi);v.clauses.forEach((c,ci)=>{let sel=state.selected&&state.selected.v==vi&&state.selected.c==ci;let marginStyle=layout.indentSide==='left'?`margin-left:${c.indent*indentPx}px`:`margin-right:${c.indent*indentPx}px`;html+=`<div class="${clauseCls(c,sel)}" data-v="${vi}" data-c="${ci}" dir="${layout.dir}" style="${marginStyle};${scriptStyle}">`;html+=renderWordTokensInClause(c.words,{vi,ci,paneState:state,activeKey,selLoc:state.selected,forExport:exportMode});html+='</div>';});html+='</div>';});return html;}
...
function insertBreak(){markUndo();let l=state.selected;if(!locOK(l)||l.w===0)return;let v=l.v,c=l.c,w=l.w;let cl=state.verses[v].clauses[c];let newc={indent:cl.indent,words:cl.words.slice(w),specials:[],ann:{}};cl.words=cl.words.slice(0,w);const isLastClause=c===state.verses[v].clauses.length-1;const hasNextVerse=v+1<state.verses.length;if(isLastClause&&hasNextVerse){state.verses[v+1].clauses.unshift(newc);applyStructureRemap(mapSplitClauseCrossVerse(v,c,w));state.selected={v:v+1,c:0,w:0};}else{state.verses[v].clauses.splice(c+1,0,newc);applyStructureRemap(mapSplitClause(v,c,w));state.selected={v,c:c+1,w:0};}render();}
function indent(delta){markUndo();let l=state.selected;if(!locOK(l))return;let v=l.v,c=l.c;let cl=state.verses[v].clauses[c];if(delta<0&&cl.indent===0){if(c>0){let prev=state.verses[v].clauses[c-1];let oldLen=prev.words.length;prev.words=prev.words.concat(cl.words);state.verses[v].clauses.splice(c,1);applyStructureRemap(mapMergeClause(v,c,c-1,oldLen));state.selected={v,c:c-1,w:oldLen};}else if(v>0){let prevV=v-1;let prevC=state.verses[prevV].clauses.length-1;let prev=state.verses[prevV].clauses[prevC];let oldLen=prev.words.length;prev.words=prev.words.concat(cl.words);state.verses[v].clauses.splice(0,1);const verseRemoved=!state.verses[v].clauses.length;applyStructureRemap(loc=>{let r=mapCrossVerseMerge(v,prevV,prevC,oldLen)(loc);if(!r)return null;return verseRemoved?mapVerseRemoved(v)(r):r;});if(verseRemoved)state.verses.splice(v,1);state.selected={v:prevV,c:prevC,w:oldLen};}else return;}else{cl.indent=Math.max(0,cl.indent+delta);state.selected=cloneLoc(l);}render();}
