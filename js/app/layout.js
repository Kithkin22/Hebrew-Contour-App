...
function renderWordTokensInClause(words,opt){const vi=opt.vi,ci=opt.ci,pane=opt.pane,paneState=opt.paneState||state,activeKey=opt.activeKey||'';const selLoc=opt.selLoc||(paneState&&paneState.selected);const idsFn=opt.idsForLoc||(pane!=null?(l=>commentIdsForLocInPane(l,pane)):(l=>commentIdsForLoc(l)));const markersFn=opt.markersForLoc||(pane!=null?(l=>commentMarkersForLocInPane(l,pane)):(l=>commentMarkersForLoc(l)));let html='';words.forEach((w,wi)=>{if(isMaqafConnector(w)){html+=`<span class="maqaf-connector" aria-hidden="true">${esc(w.text||MAQAF_CHAR)}</span>`;return;}const l=pane!=null?{v:vi,c:ci,w:wi,pane}:{v:vi,c:ci,w:wi};const ids=idsFn(l);const commentClass=ids.length?'comment-anchored':'';const activeCommentClass=(paneState.activeCommentId&&ids.includes(paneState.activeCommentId))?'comment-active':'';const wsel=selLoc&&selLoc.v===vi&&selLoc.c===ci&&selLoc.w===wi;const same=activeKey&&normalizeHebrewWord(w.text)===activeKey&&!wsel;const paneAttr=pane!=null?` data-pane="${pane}"`:'';html+=`<span class="word ${wsel?'selected':''} ${same?'sameword':''} ${commentClass} ${activeCommentClass} ${w.deleted?'deleted':''} ${w.specials.includes('predicate')?'pred':''} ${w.specials.includes('subject')?'subj':''} ${wordFormatClasses(w,l)}"${paneAttr} data-v="${vi}" data-c="${ci}" data-w="${wi}" style="${wordInlineStyle(w)}">${esc(w.text)}</span>${markersFn(l)}`;if(needsSpaceAfterWord(words,wi))html+=' ';});return html;}
...
function buildContourEditorHtmlFromState(){const layout=getLanguageLayout();const scriptStyle=`direction:${layout.dir};text-align:${layout.textAlign};font-family:${layout.fontFamily}`;ensureComments();if(!state.verses.length)return '';let html='';let activeKey=selectedWordKey();state.verses.forEach((v,vi)=>{html+=`<div class="muted" dir="ltr">${esc(v.ref)}</div>`;v.clauses.forEach((c,ci)=>{let sel=state.selected&&state.selected.v==vi&&state.selected.c==ci;let marginStyle=layout.indentSide==='left'?`margin-left:${c.indent*36}px`:`margin-right:${c.indent*36}px`;html+=`<div class="clause ${sel?'selected':''}" data-v="${vi}" data-c="${ci}" dir="${layout.dir}" style="${marginStyle};${scriptStyle}">`;html+=renderWordTokensInClause(c.words,{vi,ci,paneState:state,activeKey,selLoc:state.selected});html+='</div>';});});return html;}
