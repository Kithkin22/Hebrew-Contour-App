...
function detectUsedLegendEntries(){ensureLegend();let add=[];let sawBracket=false,sawBold=false,sawItalic=false,sawUnderline=false,sawDouble=false,sawPred=false,sawSubj=false;function scanWords(verses){verses.forEach(v=>v.clauses.forEach(c=>c.words.forEach(w=>{let f=w.format||{};if(f.highlight&&!legendHas('highlight',f.highlight))add.push({type:'highlight',color:f.highlight,label:''});if(w.color&&!legendHas('textColor',w.color))add.push({type:'textColor',color:w.color,label:''});if(w.bracketStart||w.bracketEnd)sawBracket=true;if(f.bold)sawBold=true;if(f.italic)sawItalic=true;if(f.underline)sawUnderline=true;if(f.doubleUnderline)sawDouble=true;if(w.specials&&w.specials.includes('predicate'))sawPred=true;if(w.specials&&w.specials.includes('subject'))sawSubj=true;})));}if(stateBundle.parallelEnabled){[0,1].forEach(p=>{const st=stateBundle.panes[p];if(st&&st.verses.length)scanWords(st.verses);});}else scanWords(state.verses);function maybe(type,color,label){if(!legendHas(type))add.push({type,color,label});}if(sawBracket)maybe('bracket','#ffffff','');if(sawBold)maybe('bold','#000000','');if(sawItalic)maybe('italic','#000000','');if(sawUnderline)maybe('underline','#000000','');if(sawDouble)maybe('doubleUnderline','#000000','');if(sawPred)maybe('predicate','#0b61a4','');if(sawSubj)maybe('subject','#b02a2a','');state.legend=state.legend.concat(add);renderLegendEditor();syncLegendBelowEditor();autoSaveProject();if(!add.length)alert('No new used markings found.');}
