...
function render(){applyLanguageLayout();syncStateBundle();renderParallelEditors();if(isParallelActive()){renderDualTables();}else{renderEditor();renderTable();}renderLegendEditor();syncLegendBelowEditor();renderInclusioManager();if(!isWorkspaceTableView())renderCommentsPanel();setTimeout(updateCommentPopover,0);if(isParallelActive())renderArcManagerParallel();else if(typeof renderArcManager==='function')renderArcManager();if(stateBundle.parallelEnabled)updateParallelAlignStatus();if(autosaveReady)autoSaveProject();scheduleEditorLayoutFix();}
