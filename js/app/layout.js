...
function wordFormatClasses(w,l){let f=w.format||{};let suppressCommentStart=l&&isCommentBoundary(l,'start')&&!w.bracketSource;let suppressCommentEnd=l&&isCommentBoundary(l,'end')&&!w.bracketSource;let incCls=typeof inclusioWordHighlightClass==='function'&&l?inclusioWordHighlightClass(l,w):'';let inclusioQuiet=w.bracketSource==='inclusio'?'inclusio-bracket':'';let spanCls=w.inclusioId?' inclusio-span':'';if(w.inclusioRole==='opening-start')spanCls+=' inclusio-span-start';if(w.inclusioRole==='closing-end')spanCls+=' inclusio-span-end';return `${f.bold?'fmt-bold':''} ${f.italic?'fmt-italic':''} ${f.underline?'fmt-underline':''} ${f.doubleUnderline?'fmt-double-underline':''} ${f.highlight?'fmt-highlight':''} ${(w.bracketStart&&!suppressCommentStart)?'bracket-start':''} ${(w.bracketEnd&&!suppressCommentEnd)?'bracket-end':''} ${inclusioQuiet}${spanCls} ${incCls}`.replace(/\s+/g,' ').trim();}
...
function mergeClauseWithPrevious(loc){if(!locOK(loc)||!isCaretAtClauseStart(loc))return false;const v=loc.v,c=loc.c;if(c===0&&v===0)return false;markUndo();const cl=state.verses[v].clauses[c];if(c>0){const prev=state.verses[v].clauses[c-1];const oldLen=prev.words.length;prev.words=prev.words.concat(cl.words);mergeClauseAnnInto(prev,cl);state.verses[v].clauses.splice(c,1);applyStructureRemap(mapMergeClause(v,c,c-1,oldLen));state.selected={v,c:c-1,w:oldLen};}else{const prevV=v-1;const prevC=state.verses[prevV].clauses.length-1;const prev=state.verses[prevV].clauses[prevC];const oldLen=prev.words.length;prev.words=prev.words.concat(cl.words);mergeClauseAnnInto(prev,cl);state.verses[v].clauses.splice(0,1);const verseRemoved=!state.verses[v].clauses.length;applyStructureRemap(loc=>{let r=mapCrossVerseMerge(v,prevV,prevC,oldLen)(loc);if(!r)return null;return verseRemoved?mapVerseRemoved(v)(r):r;});if(verseRemoved)state.verses.splice(v,1);state.selected={v:prevV,c:prevC,w:oldLen};}syncStateBundle();if(typeof syncAllPaneInclusioWordMarkers==='function')syncAllPaneInclusioWordMarkers();render();return true;}
