...
function wordFormatClasses(w,l){let f=w.format||{};let suppressCommentStart=l&&isCommentBoundary(l,'start')&&!w.bracketSource;let suppressCommentEnd=l&&isCommentBoundary(l,'end')&&!w.bracketSource;let incCls=typeof inclusioWordHighlightClass==='function'&&l?inclusioWordHighlightClass(l,w):'';let inclusioQuiet=w.bracketSource==='inclusio'?'inclusio-bracket':'';return `${f.bold?'fmt-bold':''} ${f.italic?'fmt-italic':''} ${f.underline?'fmt-underline':''} ${f.doubleUnderline?'fmt-double-underline':''} ${f.highlight?'fmt-highlight':''} ${(w.bracketStart&&!suppressCommentStart)?'bracket-start':''} ${(w.bracketEnd&&!suppressCommentEnd)?'bracket-end':''} ${inclusioQuiet} ${incCls}`.replace(/\s+/g,' ').trim();}
