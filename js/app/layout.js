...
function indent(delta){markUndo();let l=state.selected;if(!locOK(l))return;let v=l.v,c=l.c;let cl=state.verses[v].clauses[c];const step=typeof contourTabIndentStepPx==='function'?contourTabIndentStepPx():30;const curPx=typeof clauseIndentPx==='function'?clauseIndentPx(cl):((cl.indent||0)*step);if(delta<0&&curPx===0){if(c>0){let prev=state.verses[v].clauses[c-1];let oldLen=prev.words.length;prev.words=prev.words.concat(cl.words);state.verses[v].clauses.splice(c,1);applyStructureRemap(mapMergeClause(v,c,c-1,oldLen));state.selected={v,c:c-1,w:oldLen};}else if(v>0){let prevV=v-1;let prevC=state.verses[prevV].clauses.length-1;let prev=state.verses[prevV].clauses[prevC];let oldLen=prev.words.length;prev.words=prev.words.concat(cl.words);state.verses[v].clauses.splice(0,1);const verseRemoved=!state.verses[v].clauses.length;applyStructureRemap(loc=>{let r=mapCrossVerseMerge(v,prevV,prevC,oldLen)(loc);if(!r)return null;return verseRemoved?mapVerseRemoved(v)(r):r;});if(verseRemoved)state.verses.splice(v,1);state.selected={v:prevV,c:prevC,w:oldLen};}else return;}else{if(typeof setClauseIndentPx==='function')setClauseIndentPx(cl,curPx+delta*step);else cl.indent=Math.max(0,(cl.indent||0)+delta);state.selected=cloneLoc(l);}render();}
