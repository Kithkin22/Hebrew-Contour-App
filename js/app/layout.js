...
function syncLegendBelowEditor(){const wrap=document.getElementById('legendBelowEditor');const contourTab=document.getElementById('contourTab');if(!wrap||!contourTab)return;const anchor=isParallelActive()?document.getElementById('parallelCompareWrap'):document.getElementById('singleEditorSection');if(anchor&&wrap.parentElement===contourTab&&wrap.previousElementSibling!==anchor)anchor.insertAdjacentElement('afterend',wrap);wrap.style.display=isWorkspaceTableView()?'none':'block';wrap.style.flexShrink='0';if(state.legend&&state.legend.length)wrap.classList.remove('collapsed');const head=document.getElementById('legendBelowHeader');if(head){const paneNote=stateBundle.parallelEnabled&&isParallelActive()?' ('+paneLabel(stateBundle.activePane)+')':'';const expanded=!wrap.classList.contains('collapsed');head.textContent=(expanded?'▼':'▶')+' Legend / Key'+paneNote;}}
