...
const sandbox = {
  ...
  contourIndentDocxTwipsForClause: (c) => Math.max(0, (c.indent || 0) * 360),
  spacingAfterDocxTwips: () => 0,
  verseSpacingAfterDocxTwips: () => 120,
  contourPassageTitleForExport: () => 'Job 19:21–29',
  getAlephTranslationForVerse(v) {
    const m = String(v.ref || '').match(/(\d+)\s*:\s*(\d+)/);
    const cv = m ? +m[1] + ':' + +m[2] : '';
    const store = sandbox.state && sandbox.state.alephTranslations;
    if (!store || !store.byChapterVerse || !store.byChapterVerse[cv]) return '';
    return store.byChapterVerse[cv].text || '';
  },
};
...
sandbox.state = {
  ...
  language: 'hebrew',
  verses: [
    { ref: 'Job 19:21', clauses: [clause(['חָנֻּנִי', 'חָנֻּנִי', 'אַתֶּם', 'רֵעָי'], 0), clause(['כִּי', 'יַד־אֱלוֹהַּ', 'נָגְעָה', 'בִּי'], 1)] },
    { ref: 'Job 19:22', clauses: [clause(['לָמָּה', 'תִרְדְּפֻנִי', 'כְמוֹ־אֵל'], 0), clause(['וּמִבְּשָׂרִי', 'לֹא', 'תִשְׂבָּעוּ'], 1)] },
    ...
    { ref: 'Job 19:25', clauses: [clause(['וַאֲנִי', 'יָדַעְתִּי', 'גֹּאֲלִי', 'חָי'], 0), clause(['וְאַחֲרוֹן', 'עַל־עָפָר', 'יָקוּם'], 1)] },
...
const checks = {
  ...
  threeWidths: documentXml.includes('w:tblW w:w="5000" w:type="pct"') && documentXml.includes('6200') && documentXml.includes('w:w="500"') && documentXml.includes('4100'),
  engTypography:
    documentXml.includes('w:sz w:val="23"') &&
    documentXml.includes('w:line="240"') &&
    documentXml.includes('w:after="160"') &&
    documentXml.includes('Be gracious to me, be gracious to me,') &&
  ...
  hebrewFlushRight:
    ...
    documentXml.includes('<w:tblInd w:w="0"') &&
    /w:tcW w:w="4100"[\s\S]*?w:right w:w="0"/.test(documentXml) &&
    /w:tblCellMar>[\s\S]*w:right w:w="0"/.test(documentXml),
  naturalReflow: !documentXml.includes('<w:br/>') && documentXml.includes('as the hand of God has struck me.'),
  englishFirst: documentXml.indexOf('Be gracious') < documentXml.indexOf('חָנֻּנִי'),
  verseNums: documentXml.includes('>21<') && documentXml.includes('>29<'),
  cantSplit: documentXml.includes('<w:cantSplit/>'),
  topAlign: documentXml.includes('<w:vAlign w:val="top"/>'),
  rtl: documentXml.includes('<w:bidi/>'),
};
